...
export const PostDetailPage = () => {
  ...
  return (
    <motion.article
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="mx-auto max-w-4xl px-6 py-12 text-neutral-900 transition-colors dark:text-neutral-100"
    >
      <Link
        to="/"
      ...
      <header className="mb-12 space-y-6">
        ...
        <h1 className="text-4xl font-bold leading-tight text-neutral-900 dark:text-neutral-100 md:text-5xl">{post.title}</h1>

        {post.excerpt ? (
          <p className="text-xl leading-relaxed text-neutral-600 dark:text-neutral-300">{post.excerpt}</p>
        ) : null}
      </header>
      ...
      <div className="prose prose-lg max-w-none pb-12 text-neutral-800 dark:prose-invert">
...
const markdownComponents: Components = {
  ...
  code: ({ inline, ...props }: { inline?: boolean } & HTMLAttributes<HTMLElement>) =>
    inline ? (
      <code className="rounded bg-neutral-100 px-2 py-1 text-sm text-accent dark:bg-neutral-800" {...props} />
    ) : (
